...
if [ ! -f "$FRAMEWORK/Makefile" ]; then
    echo "SKIP  the PihlCore Test framework is not checked out at $FRAMEWORK/."
...
for makefile in lectures/L*/exercises/test/Makefile; do
    ...
    if ! make -C "$dir" build \
        AEL_DIR="$toolkit" \
        PIHLCORE_TEST_DIR="$(pwd)/$FRAMEWORK" >"$build_log" 2>&1; then
    ...
    if make -C "$dir" run AEL_DIR="$toolkit" PIHLCORE_TEST_DIR="$(pwd)/$FRAMEWORK"; then
        passed=$((passed + 1))
    else
        failed=$((failed + 1))
    fi
